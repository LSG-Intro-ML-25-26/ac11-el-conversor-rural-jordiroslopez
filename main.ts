...
let maquina_expendedora = sprites.create(assets.image`miImagen`, SpriteKind.Machine)
...
game.onUpdate(function () {
    if (nena.overlapsWith(maquina_expendedora)) {
        boton = sprites.create(assets.image`miImagen0`, SpriteKind.Button)
    ...
    if (arbol.overlapsWith(nena)) {
        ...
        if (controller.A.isPressed()) {
            sprites.destroyAllSpritesOfKind(SpriteKind.Tree, effects.spray, 100)
            generar_arbol()
        }
    }
})
